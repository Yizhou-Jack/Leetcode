...
public class Solution {
    ...
    当0出现时，如果0前面的子数组是[4,3,2,1]（一个逐1减小的子数组），那么会造成无法到达的结果（因为不管怎么跳跃都会到达0）。
    只要有一个数字比当前所在位置逐渐增大的结果要大（比如前面的子数组是[4,3,2,2]，本来末尾1的位置变成了2），那么在这个位置能够跳过0，继续跳跃至最后一个位置。

    因此可以从末尾开始遍历此数组，碰到0时遍历0前面的子数组，如果发现出现了能够跳过0的情况，那么就忽略这个0，继续向前找其他的0。
    如果最后能遍历完这个数组，说明可以到达最后一个位置。代码中采用index来记录遍历的位置，用count来记录逐渐减小的数组的减小大小。
    */
